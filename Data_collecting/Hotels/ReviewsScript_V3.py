...
Links_csv = r'C:\Users\Kariem\Desktop\New_folder\Smart-Tourism-Dev-Sys\Data_collecting\Hotels\new_test\hotels_links.csv'
HOTEL_DATA_FILE = r'C:\Users\Kariem\Desktop\New_folder\Smart-Tourism-Dev-Sys\Data_collecting\Hotels\new_test\hotels_data.csv'
REVIEWS_FILE = r'C:\Users\Kariem\Desktop\New_folder\Smart-Tourism-Dev-Sys\Data_collecting\Hotels\new_test\hotels_reviews3.csv'
...
def process_hotels_incrementally_from_file():
    ...
    for idx, hotel in enumerate(hotel_links):
        # Check the flag; note: CSV booleans may come as strings.
        if not str(hotel.get('Is_Scraped', False)).lower() in ["true", "1"]:
            ...
            try:
                ...
                try:
                    cat_dict = {}
                    # Find all category containers having the common classes and a review subscore element.
                    ...
                    for cat in category_elements:
                        try:
                            # Get the label element (without "bdc1ea4a28")
                            ...
                            # Get the rating value (element that has "bdc1ea4a28")
                ...
                # Process reviews: click "Read All Reviews" if available
                try:
                ...
                # Process reviews on the current page and then check for next pages.
                while True:
                    ...
                    # Try to find the "next page" button for reviews.
                    try:
                        # Using the provided HTML snippet, we target the next-page button.
                        next_page_btn = driver.find_element(By.CSS_SELECTOR, 'span.eedba9e88a > span.fcd9eec8fb')
                        next_page_btn.click()
                        sleep(3)  # Wait for the next review page to load
                    except Exception as e:
                        print(f"No further review pages for {hotelName}: {e}")
                        break
...
if __name__ == "__main__":
    ...
    # Step 1: Scrape hotel links per city if needed.
...
#1- handle multiple pages of reviews
#2- handle multiple pages of hotels
#3- refine city name extraction 
#4- refine the way of extracting the category ratings
