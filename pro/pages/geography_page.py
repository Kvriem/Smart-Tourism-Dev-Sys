...
def get_city_color_scheme():
    """Define professional high-contrast colors with excellent visual distinction"""
    return {
        'Cairo': '#e11d48',         # Rose Red - Rich and professional
        'Luxor': '#2563eb',         # Blue - Clear and bright
        'Alexandria': '#059669',    # Emerald Green - Professional green
        'Aswan': '#d97706',         # Amber Orange - Warm and distinct
        'Hurghada': '#7c3aed',      # Violet Purple - Rich purple
        'Sharm El Sheikh': '#0891b2', # Sky Blue - Ocean theme
        'Dahab': '#be185d',         # Pink - Vibrant contrast
        'Marsa Alam': '#065f46',    # Dark Emerald - Deep contrast
        'Safaga': '#ea580c',        # Orange - Warm professional
        'Taba': '#581c87',          # Deep Purple - Royal tone
        'Edfu': '#166534',          # Forest Green - Natural tone
        'Kom Ombo': '#b91c1c',      # Dark Red - Strong contrast
        'Esna': '#1e40af',          # Blue - Professional tone
...
def create_city_legend_cards(cities, title_prefix="Cities"):
    """Create color legend cards for cities"""
    city_colors = get_city_color_scheme()
    ...
    for city in sorted(cities):
        color = city_colors.get(city, '#374151')  # Professional gray fallback
    ...
    return html.Div([        html.Div([
            html.Span([
                html.I(className="fas fa-palette me-2", style={'color': '#2563eb'}),
                f"{title_prefix} Color Legend:"
            ], style={
                'fontSize': '0.95rem', 
                'fontWeight': '600', 
                'color': '#374151', 
...
def create_top_tokens_by_city_chart(df, token_type='positive', top_n=5):
    ...
    # Get distinct colors for each city
    city_colors = get_city_color_scheme()
    cities_in_data = tokens_df['City'].unique()    # Create color mapping for cities in the data
    color_discrete_map = {}
    for city in cities_in_data:
        color_discrete_map[city] = city_colors.get(city, '#374151')  # Professional gray fallback
    ...
    fig = px.bar(
        ...
        y='Count',
        color='City',
        title=f"🔍 Top {token_type.capitalize()} Keywords by City - Color Coded Analysis",
        barmode='group',
      ...
      # Enhanced styling with better contrast and visual appeal
    fig.update_traces(
        texttemplate='<b>%{text}</b>',
        textposition='outside',
        textfont_size=11,
        textfont_color='#1f2937',  # Dark gray text for better readability
        textfont_family='Inter, sans-serif',
        marker_line=dict(width=1.5, color='rgba(255,255,255,0.9)'),  # Clean white border
        marker=dict(
            line=dict(width=1, color='rgba(0,0,0,0.08)'),  # Subtle dark border
            # Add subtle shadow effect
            opacity=0.95
        ),
        hovertemplate='<b>City:</b> %{fullData.name}<br>' +                     '<b>Keyword:</b> %{x}<br>' +
                     '<b>Frequency:</b> %{y}<br>' +
                     '<extra></extra>'
    )
    
    fig.update_layout(
        plot_bgcolor='rgba(248,250,252,0.5)',  # Very light gray background
        paper_bgcolor='rgba(255,255,255,1)',     # Pure white paper background
        font=dict(family="Inter, sans-serif", size=12, color='#374151'),
        title=dict(
            font=dict(size=20, weight='bold', color='#1f2937'),  # Professional dark gray title
            x=0.5,
            xanchor='center',
            pad=dict(t=20, b=20)
        ),
        height=520,  # Optimized height
        width=1200,  # Maintained width for spacing
        xaxis=dict(
            title=f'{token_type.title()} Keywords',
            title_font=dict(size=14, weight='bold', color='#374151'),
            tickfont=dict(size=11, color='#4b5563'),
            tickangle=-30,  # Better readability angle
            tickmode='linear',
            showgrid=True,
            gridcolor='rgba(156,163,175,0.15)',  # Lighter grid lines
            zeroline=False,
            showline=True,
        ...
        yaxis=dict(
            ...
            title_font=dict(size=14, weight='bold', color='#374151'),
            tickfont=dict(size=11, color='#4b5563'),
            gridcolor='rgba(156,163,175,0.2)',  # Lighter grid
            zeroline=True,
        ...
        legend=dict(
            ...
            title=dict(text='<b>Cities</b>', font=dict(size=13, color='#1f2937'))
        ),
        margin=dict(l=80, r=160, t=100, b=120),
        showlegend=True,
        # Better spacing between bars for clarity
        bargap=0.2,
        bargroupgap=0.15
    )
                    ...
                    dbc.Card([
                        dbc.CardBody([
                            html.Div([
                                ...
                                html.H4("Cities Analyzed", className="kpi-card-title"),
                                html.H2(id="cities-overview", className="kpi-card-value")
                            ], className="kpi-card-content")
                        ], className="kpi-card-body")
                    ], className="kpi-stat-card kpi-cities-card")
                ], width=3, className="kpi-card-column"),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.Div([
                                ...
                                html.H4("Total Reviews", className="kpi-card-title"),
                                html.H2(id="total-reviews", className="kpi-card-value")
                            ], className="kpi-card-content")
                        ], className="kpi-card-body")
                    ], className="kpi-stat-card kpi-reviews-card")
                ], width=3, className="kpi-card-column"),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.Div([
                                ...
                                html.H4("Hotels Covered", className="kpi-card-title"),
                                html.H2(id="total-hotels", className="kpi-card-value")
                            ], className="kpi-card-content")
                        ], className="kpi-card-body")
                    ], className="kpi-stat-card kpi-hotels-card")
                ], width=3, className="kpi-card-column"),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.Div([
                                ...
                                html.H4("Overall Sentiment", className="kpi-card-title"),
                                html.H2(id="overall-sentiment", className="kpi-card-value")
                            ], className="kpi-card-content")
                        ], className="kpi-card-body")
                    ], className="kpi-stat-card kpi-sentiment-card")
                ], width=3, className="kpi-card-column")
            ], className="kpi-cards-row"),
        ...
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    ...
                    dbc.CardBody([
                        html.Div(id="positive-tokens-legend", className="mb-3"),
                        dcc.Graph(
            ...
            dbc.Col([
                dbc.Card([
                    ...
                    dbc.CardBody([
                        html.Div(id="negative-tokens-legend", className="mb-3"),
                        dcc.Graph(
...
@callback(
    [Output('sentiment-by-city-chart', 'figure'),
     ...
     Output('positive-tokens-legend', 'children'),
     Output('negative-tokens-legend', 'children'),
...
def update_geography_charts(selected_cities):
    ...
    if df.empty:
        ...
        return empty_fig, empty_fig, empty_fig, html.Div(), html.Div(), empty_fig, empty_fig, empty_fig, empty_fig, "No data available for analysis."
    ...
    if df_filtered.empty:
        ...
        return empty_fig, empty_fig, empty_fig, html.Div(), html.Div(), empty_fig, empty_fig, empty_fig, empty_fig, "No data available for selected cities."    # Create charts with enhanced token display
    ...
    nationality_satisfaction_fig = create_nationality_satisfaction_chart(df_filtered)
      # Create legend cards for the token charts
    cities_in_data = df_filtered['City'].unique()
    positive_legend = create_city_legend_cards(cities_in_data, "Cities in Positive Keywords Chart")
    negative_legend = create_city_legend_cards(cities_in_data, "Cities in Negative Keywords Chart")
