...
def create_satisfaction_trend_chart(df, city_filter="all", start_date=None, end_date=None):
    ...
    fig.update_layout(
        title={
            'text': "Satisfaction Trend Over Time",
            'x': 0.5,
            'xanchor': 'center',
    ...
    fig.update_yaxes(
        showgrid=True,
        gridwidth=1,
        gridcolor='rgba(0,0,0,0.1)',        showline=True,
        linewidth=1,
        linecolor='rgba(0,0,0,0.2)',
        range=[0, 100]
    )
...
def generate_government_insights(word, word_type, mentions, reviews, cities_count, cities, sentiment_score):
    ...
    if mentions >= 50:
        impact_level = "High"
        insights.append(f"🎯 **High Impact Alert**: '{word}' appears {mentions} times across {reviews} reviews, indicating significant public attention.")
    elif mentions >= 20:
        impact_level = "Medium"
        insights.append(f"📊 **Medium Impact**: '{word}' is mentioned {mentions} times, suggesting moderate public concern or satisfaction.")
    else:
        impact_level = "Low"
        insights.append(f"📝 **Low Impact**: '{word}' has {mentions} mentions, representing a specific but limited public sentiment.")
    
    # Geographic distribution insights
    if cities_count > 1:
        insights.append(f"🌍 **Geographic Spread**: This sentiment spans across {cities_count} cities ({', '.join(cities[:3])}{'...' if len(cities) > 3 else ''}), indicating region-wide implications.")
    else:
        insights.append(f"📍 **Localized Issue**: This sentiment is primarily concentrated in {cities[0] if cities else 'specific areas'}, suggesting targeted intervention opportunities.")
    
    # Sentiment impact analysis
    if word_type == 'positive':
        if abs(sentiment_score) > 5:
            insights.append(f"✅ **Positive Driver**: This word contributes {abs(sentiment_score):.1f}% to overall satisfaction, representing a key strength to maintain and promote.")
        insights.append(f"🚀 **Success Factor**: Citizens associate '{word}' with positive experiences, indicating successful service delivery in this area.")
    else:
        if abs(sentiment_score) > 5:
            insights.append(f"⚠️ **Critical Issue**: This word negatively impacts {abs(sentiment_score):.1f}% of satisfaction, requiring immediate government attention.")
        insights.append(f"🔧 **Improvement Opportunity**: Citizens express dissatisfaction with '{word}', highlighting a priority area for policy intervention.")
    
    # Urgency assessment
    if word_type == 'negative' and mentions > 30:
        insights.append(f"🚨 **Urgent Action Required**: High frequency of negative mentions suggests this issue significantly affects citizen satisfaction and requires priority attention.")
    elif word_type == 'positive' and mentions > 30:
        insights.append(f"🏆 **Success Story**: High positive mentions indicate this is a model practice that should be replicated across other areas and services.")
    
    return insights

def generate_government_recommendations(word, word_type, mentions, reviews, cities, associated_words):
    """Generate actionable government recommendations"""
    recommendations = []
    
    if word_type == 'positive':
        # Recommendations for positive words
        ...
        # Recommendations for negative words
        recommendations.extend([
            {
...
def create_nationality_modal_content(nationality_data):
    ...
    return html.Div([
        # Summary Statistics Cards (Compact 4-card layout)
        html.Div([
            html.H5([
...
def create_modal_content(word_data):
    ...
    color_scheme = {
        ...
        'light': '#d1fae5' if word_type == 'positive' else '#fee2e2',
        'icon': 'fa-thumbs-up' if word_type == 'positive' else 'fa-thumbs-down'
    }
    
    return html.Div([        # Summary Statistics Cards (Compact 4-card layout)
        html.Div([
            html.H5([
                html.I(className=f"fas fa-chart-bar me-2", style={"color": color_scheme['primary']}),
                "Key Statistics"
...
def generate_nationality_recommendations(nationality, satisfaction_rate, cities_visited, top_liked, top_disliked):
    """Generate recommendations for nationality-specific improvements"""
    recommendations = []
    ...
    if len(cities_visited) == 1:
        ...
        recommendations.append({
            ...
            'expected_impact': 'Increase visitor volume and market penetration'
        })
    else:
        recommendations.append({
            ...
            'expected_impact': 'Enhance visitor experience and extend stay duration'
        })
    ...
    if satisfaction_rate < 70:
        recommendations.append({
            ...
            'priority': 'High',
            'action': f"Implement cultural sensitivity training for staff serving {nationality} guests",
            'expected_impact': 'Improve satisfaction rates and cultural understanding'
        })
    
    if nationality not in ['United States', 'United Kingdom', 'Australia', 'Canada']:        recommendations.append({
            'category': 'Communication',
            'priority': 'Medium',
            'action': f"Consider multilingual support or cultural liaison services for {nationality} guests",
            'expected_impact': 'Improve communication and cultural comfort'
        })
